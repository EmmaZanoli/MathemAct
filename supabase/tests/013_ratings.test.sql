-- Propositions, ratings, and the aggregate.
--
...
insert into public.propositions (id, author_id, statement, area, status, activated_at)
values
  ...
  ('cccc0000-0000-0000-0000-000000000003', 'bbbb0000-0000-0000-0000-000000000001',
   'This proposition has been moderated away and nobody should see it.', 'other', 'hidden', null);

-- ── The scale ───────────────────────────────────────────────────────────────────────

select throws_ok(
  $$ insert into public.ratings (proposition_id, user_id, score)
...
select is(
  (select count(*)::int from public.ratings
    where proposition_id = 'cccc0000-0000-0000-0000-000000000001'),
  1,
...
select throws_ok(
  ...
  'one rating per person per proposition'
);
...
select throws_ok(
  ...
  '42501'::text, null::text,
  'a hidden proposition stops accepting ratings'
);
...
select is(
  (select count(*)::int from public.propositions),
  2,
  'anon sees proposed and active propositions, and not hidden ones'
);
...
select throws_ok(
  $$ insert into public.propositions (author_id, statement, area, status)
...
select is(
  (select status::text from public.propositions
...
insert into public.ratings (proposition_id, user_id, score) values
...
select is(
  ...
  'proposed'::text,
  'below the threshold, a proposition stays proposed'
);
...
select isnt(
  (select activated_at from public.propositions
    where id = 'cccc0000-0000-0000-0000-000000000002'),
  null::timestamptz,
...
-- ── The aggregate ───────────────────────────────────────────────────────────────────
-- Six raters on the active proposition: scores 7, 2, 8, 8, and two who declined.
...
select is(
  (select histogram from public.proposition_ratings
    where proposition_id = 'cccc0000-0000-0000-0000-000000000001'),
...
select is(
  (select median from public.proposition_ratings
    where proposition_id = 'cccc0000-0000-0000-0000-000000000001'),
  7::smallint,
...
select is(
  (select total_raters from public.proposition_ratings
    where proposition_id = 'cccc0000-0000-0000-0000-000000000001'),
  4,
...
select is(
  (select no_opinion_count from public.proposition_ratings
    where proposition_id = 'cccc0000-0000-0000-0000-000000000001'),
  1,
...
select is(
  (select coverage from public.proposition_ratings
    where proposition_id = 'cccc0000-0000-0000-0000-000000000001'),
  0.750::numeric,
...
select is_empty(
  $$ select proposition_id from public.proposition_ratings
      where proposition_id = 'cccc0000-0000-0000-0000-000000000003' $$,
  'a hidden proposition has no readable aggregate'
);
